...
🟠 **F5.2 — живой киоск не пишет в ожидаемый лог.** Файл вытащен при РАБОТАЮЩЕМ киоске (включили,
потыкали, скопировали) и оказался нулевым до последнего байта. При `O_APPEND` данные текущей сессии
были бы в конце — их нет → работающий процесс в этот файл не пишет. Два механизма (развести поиском по
`C:\Users`): **(A)** киоск (shell-replacement через watchdog + автологин) стартует под другой
сессией/учёткой → `os.UserCacheDir()` указывает на чужой профиль, и живой лог лежит там, а вытащенный
файл — старый desktop-лог, обнулённый прошлым hard-reset (объясняет и отсутствие папки `GameSaver` в
профиле desktop-пользователя); **(B)** `logging.Setup` отвалился, но `main.go:41` только логирует ошибку
и продолжает без файлового хендлера. Для приложения, у которого киоск — основной режим, это делает
логи бесполезными именно в бою.

**Подтверждено поиском по диску (2026-07-07):** найдено три `gamesaver.log`:
`C:\Users\Администратор\AppData\Local\GameSaver\logs\` (210047 Б, **весь нули** — это и есть
присланный файл) + два в MSIX-редиректе `...\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\GameSaver\`
(Администратор 13594 Б и LastSkywalker 56319 Б — с данными). Итог: **киоск работает под учёткой
`Администратор`** (автологин), поэтому в профиле desktop-пользователя лог не появлялся; боевой
киоск-лог обнулён (F5.1 подтверждён — жёсткое выключение без `fsync`).

**Варианты решения:** `f.Sync()` после записи (или периодический flush по таймеру); ротация;
дублирование критичных ошибок в Windows Event Log; **фиксированный путь лога независимо от профиля/MSIX**
(напр. рядом с exe или в общей `%ProgramData%\GameSaver\logs`), чтобы киоск и desktop писали в одно
место; на ошибке `logging.Setup` в киоске — не глотать, а показывать на экране. До фикса — снимать лог
только после чистого закрытия / без hard-reset.

🔴 **F5.3 — данные размазаны по профилям и MSIX-контейнерам (риск рассинхрона библиотеки/бэкапов).**
Путь данных полностью определяется `os.UserCacheDir()`/`os.UserConfigDir()` (`config.go:59-82`), которые
зависят от (а) учётки процесса и (б) MSIX-редиректа. На этой машине лог нашёлся в трёх местах: профиль
`Администратор` (киоск), профиль `LastSkywalker` (desktop) и оба — ещё раз внутри пакета
`Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local`. По той же логике разъезжаются **`gamesaver.db`,
settings.json, бэкапы (`BackupRoot` дефолт `C:\GameSavesBackup` — исключение, он абсолютный), secrets**.
Следствие: в desktop-режиме и в киоске приложение может открывать **разные БД** → разная
библиотека/матчи/история бэкапов; бэкап, сделанный в одном режиме, не виден в другом (кроме reconcile
по `C:\GameSavesBackup`). Для приложения, где киоск и desktop — два штатных режима одной установки, это
скрытый источник «пропавших» данных.

**Варианты решения:** привязать данные к фиксированному, независимому от учётки/MSIX пути
(рядом с exe для портативной раздачи, либо `%ProgramData%\GameSaver` с общим ACL), либо явно
документировать и синхронизировать; как минимум — писать в лог фактический `UserDataDir` при старте,
